...
public interface CustomerMyIsamRepository extends JpaRepository<CustomerMyISAM, Integer> {
    ...
    @Query(value = "INSERT INTO customer_myisam (name, surname, email, password) VALUES (:name, :surname, :email, :password)", nativeQuery = true)
    @Modifying
    ...
    @Query(value = "SELECT * FROM customer_myisam AS c WHERE c.id = LAST_INSERT_ID() AND c.email = :email", nativeQuery = true)
    CustomerMyISAM getLastInsertedCustomer(@Param ("email") String email);
