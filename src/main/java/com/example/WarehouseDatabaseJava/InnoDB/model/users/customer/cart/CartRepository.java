...
public interface CartRepository extends JpaRepository<Cart, Integer> {
    ...
    @Query(value = "SELECT new com.example.WarehouseDatabaseJava.dto.cart.CartProductDTO (p.id, p.name, cp.quantity, cp.price) FROM CartProduct cp JOIN Cart c ON cp.cart.id = c.id JOIN Product AS p ON cp.product.id = p.id WHERE c.customer.id = :customer_id")
