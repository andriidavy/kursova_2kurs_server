...
public interface CustomProductRepository extends JpaRepository<CustomProduct, Integer> {
    @Query("SELECT new com.example.WarehouseDatabaseJava.dto.custom.CustomProductDTO(cp.product.id, p.name, cp.quantity) FROM CustomProduct cp JOIN Product p ON cp.product.id = p.id WHERE cp.custom.id = :custom_id")
